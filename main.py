...
ui.upload(on_upload=lambda e: ui.notify(f"Uploaded {e.file.name}")).classes(
    "max-w-full"
)

ui.button("Sortierung starten", on_click=lambda: start_sorter).props("icon=play_arrow")
...
def importiere_pdf():
    list(ordner.glob("*pdf"))


def start_sorter():
    ui.notify("Sortierung gestartet")
